...
import { getSheet } from '../services/sheetsService';
import { getUserSubmissions } from '../services/submissionService';

const SheetDetail = ({ user }) => {
  ...
  // Fetch user submissions for progress tracking (same as Practice.js)
  ...
  const fetchSheetData = useCallback(async () => {
    ...
    try {
      ...
      setQuestions(problemsList);

      // Extract available tags
      const tagsSet = new Set();
      problemsList.forEach(question => {
  ...
  }, [fetchUserSubmissions, fetchSheetData]);
  ...
  }, [questions, searchTerm, filters, userSubmissions, user]);
      ...
      <div className="min-h-screen bg-gray-50 pb-24">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="text-center">
            <h2 className="text-2xl font-bold text-gray-900">Sheet not found</h2>
            <Link to="/practice/sheets" className="text-blue-600 hover:text-blue-800 mt-4 inline-block">
              ← Back to Sheets
            </Link>
          </div>
        </div>
      ...
      <div className="bg-white border-b">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="flex items-center gap-2 mb-4">
            <Link to="/practice/sheets" className="text-blue-600 hover:text-blue-800">
              ← Back to Sheets
            </Link>
          </div>
            ...
            <>
              ...
              <div className="mt-6 bg-gradient-to-r from-green-50 to-blue-50 border border-green-200 rounded-lg p-4">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-sm font-medium text-gray-700">Sheet Progress</span>
                ...
                <div className="w-full bg-gray-200 rounded-full h-3 mb-2">
                  <div
                    ...
                    style={{ width: `${stats.total > 0 ? Math.round((stats.solved / stats.total) * 100) : 0}%` }}
                  />
                </div>
                <div className="flex items-center justify-between text-sm text-gray-600">
                  <span>{stats.solved} of {stats.total} problems solved</span>
                  {sheet.estimated_hours && (
                    <span className="flex items-center gap-1">
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                      </svg>
                      ~{sheet.estimated_hours}h
                    </span>
                  )}
                </div>
              </div>
