  ...
  const loadCodeForLanguage = useCallback(async (questionData, lang, availableTestCases = []) => {
    ...
    try {
      ...
      if (typeof draftCode === 'string' && draftCode.length > 0) {
        ...
        const template = generateCodeTemplate(questionData, lang, availableTestCases);
        setCode(template);
        lastSavedRef.current = template;
      }
    } catch (draftError) {
      console.error('Failed to load code draft:', draftError);
      const fallbackTemplate = generateCodeTemplate(questionData, lang, availableTestCases);
                  ...
                  <div className="bg-gray-50 dark:bg-gray-900 rounded-lg p-4 border border-gray-200 dark:border-gray-700">
                    ...
                    <div className="space-y-3">
                      {testCases
                        ...
                        .slice(0, 4)
                        .map((testCase, index) => (
                        <div key={index} className="p-3 rounded border bg-white dark:bg-gray-900">
                    ...
                    {testCases.filter(tc => !tc.hidden).length > 4 && (
                      <div className="mt-3 text-xs text-blue-600 dark:text-blue-300">
                        <p>+{testCases.filter(tc => !tc.hidden).length - 4} more visible test cases will be used during submission</p>
                      </div>
                    )}
                  </div>
                </div>
