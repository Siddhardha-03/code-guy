# Firebase Setup Guide (Updated for Unified Auth Flow)

## Overview
Firebase provides authentication (identity, email verification, password resets). The server trusts Firebase ID tokens and maps them to MySQL `users` rows. No legacy JWT issuance is needed anymore.

## Steps
1. Create Firebase project (already: `code-guy`).
2. Enable Email/Password provider in Firebase Console > Authentication.
3. Download Service Account key:
   - Firebase Console > Project Settings > Service Accounts > Generate new private key.
   - Save to `server/config/serviceAccountKey.json` (keep it out of version control).
4. Add environment variables (client):
...
5. Add server environment variables for DB + optional `CLIENT_URL`.

## Auth Lifecycle
```text
Client Signup/Login (Firebase) -> ID Token -> POST /api/auth/sync (Authorization: Bearer <token>)
   -> Server verifies token (firebase-admin) -> creates/updates MySQL user
Client requests /api/auth/me (Bearer token) -> Server returns unified profile
```

## Email Verification
On signup we send verification email. Until verified, UI restricts certain actions. Once user clicks verification link and reloads, Firebase `emailVerified` becomes true; next `/auth/sync` updates `email_verified` in MySQL.

## Admin Promotion
Use script:
```powershell
node server/make-admin.js user@example.com
```
This sets MySQL role and Firebase custom claim `{ admin: true }`. Custom claim propagates on next token refresh (user must re-login).

Alternatively call endpoint (as existing admin):
```http
...
Client auto-refreshes token every ~50 minutes. If server responds `401 TOKEN_EXPIRED`, AuthContext forces a refresh and retries once.

## Deprecated Endpoints
`/api/auth/register`, `/api/auth/login`, `/api/auth/profile`, `/api/auth/update-profile` now return HTTP 410 and should not be used.

## Testing
Basic test scripts:
- `node server/test-firebase-auth.js` (smoke)
- `node server/test-auth-flow.js` (sync + profile + admin checks)

Provide `TEST_USER_ID_TOKEN` (and optionally `ADMIN_ID_TOKEN`) in environment for `test-auth-flow.js`.

## Security Notes
- Always send Firebase ID token in `Authorization: Bearer <token>` header.
- Never trust UID/email from request body.
- Service account key must stay private; add to `.gitignore`.

## Next Enhancements
- Remove deprecated endpoints completely after transition window.
- Optional Firebase Emulator integration for local automated tests.

## Troubleshooting
| Issue | Cause | Resolution |
|-------|-------|------------|
| 401 TOKEN_EXPIRED | Token past 1h expiry | Client auto refresh; if persists, re-login |
| 410 Deprecated endpoint | Legacy route hit | Migrate client to Firebase flow |
| Admin claim not applied | Token not refreshed | Sign out/in after promotion |
| User role mismatch | Claim set but DB not yet synced | Next protected route hit triggers role sync |

## Reference Files
- `client/src/config/firebase.js`
- `client/src/contexts/AuthContext.js`
- `server/middlewares/authFirebase.js`
