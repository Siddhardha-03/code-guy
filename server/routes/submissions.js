...
router.post('/run', verifyFirebaseToken, async (req, res) => {
  try {
    ...
    // Debug: log incoming run request
    console.log('[RUN] Incoming run request:', {
      questionId,
      testCaseId,
      language,
    ...
    // Debug: log judge0 raw result summary
    console.log('[RUN] Judge0 result summary:', {
      status: result.status,
      statusId: result.statusId,
      stdoutSample: (result.stdout || '').slice(0, 200),
      stderrSample: (result.stderr || result.compileOutput || result.message || '').slice(0, 200)
    });
 ...
 * @access  Private
 */
router.post('/:questionId', verifyFirebaseToken, async (req, res) => {
  try {
 ...
 * @access  Private
 */
router.get('/question/:questionId', verifyFirebaseToken, async (req, res) => {
  try {
    const questionId = req.params.questionId;
    let page = parseInt(req.query.page, 10);
    if (!Number.isInteger(page) || page < 1) page = 1;
