...
router.get('/', (req, res, next) => {
  // Try to verifyFirebaseToken, but don't fail if no token
 ...
 * @access  Private (Admin only)
 */
router.post('/:id/questions', verifyFirebaseToken, isAdmin, async (req, res) => {
  try {
    const quizId = req.params.id;
 ...
 * @desc    Submit quiz answers (optionally in contest context)
 * @access  Private
 */
router.post('/:id/submit', verifyFirebaseToken, async (req, res) => {
  try {
    const quizId = req.params.id;
    const { answers, contestId, contestItemId } = req.body;
    // If contest context provided, verify participant is still active
    if (contestId) {
      try {
        const [parts] = await req.db.execute(
          'SELECT status FROM contest_participants WHERE contest_id = ? AND user_id = ?',
          [contestId, req.user.id]
        );
        if (!parts || parts.length === 0) {
          return res.status(403).json({ status: 'error', message: 'You are not registered for this contest' });
        }
        const status = parts[0].status;
        if (status !== 'active') {
          return res.status(403).json({ status: 'error', message: 'Contest participation is finalized; submissions are not allowed' });
        }
      } catch (e) {
        console.error('Error checking participant status:', e);
      }
    }
    console.log('Received quiz submit request', { quizId, userId: req.user?.id, contestId, contestItemId });
    ...
    // If submitted as part of a contest, attempt to record in contest_submissions table
    if (contestId) {
      try {
        let contestItemToUse = contestItemId;

        // If contestItemId wasn't provided, try to find the contest_item by quiz id
        if (!contestItemToUse) {
          const [found] = await req.db.execute(
            'SELECT id FROM contest_items WHERE contest_id = ? AND item_type = ? AND item_id = ? LIMIT 1',
            [contestId, 'quiz', quizId]
          );
          if (found && found.length > 0) contestItemToUse = found[0].id;
        }

        if (contestItemToUse) {
          // Get points for this contest item
          const [[itemDetails]] = await req.db.execute(
            'SELECT points FROM contest_items WHERE id = ?',
            [contestItemToUse]
          );
          const itemPoints = itemDetails?.points || 100;
          
          // Calculate quiz score (all or nothing for now, or proportional)
          const quizScore = Math.round((score / totalQuestions) * itemPoints);
          
          // Record contest quiz submission. Include answers and total_questions.
          await req.db.execute(
            `INSERT INTO contest_submissions 
             (contest_id, contest_item_id, user_id, item_type, item_id, answers, score, total_questions, passed) 
             VALUES (?, ?, ?, 'quiz', ?, ?, ?, ?, ?)
             ON DUPLICATE KEY UPDATE
            ...
            [contestId, contestItemToUse, req.user.id, quizId, JSON.stringify(answers), quizScore, totalQuestions, score === totalQuestions ? 1 : 0]
          );
          ...
          await req.db.execute(
            ...
            [contestId, req.user.id, contestId, req.user.id]
          );
        }
      } catch (contestError) {
        // Log but don't fail if contest submission recording fails
        console.error('Error recording contest quiz submission:', contestError?.message || contestError);
      }
    }
 ...
 * @access  Private
 */
router.get('/submissions/user', verifyFirebaseToken, async (req, res) => {
  try {
 ...
 * @access  Private (Admin only)
 */
router.get('/:id/results', verifyFirebaseToken, isAdmin, async (req, res) => {
  try {
    const quizId = req.params.id;
 ...
 * @access  Private
 */
router.get('/stats/user', verifyFirebaseToken, async (req, res) => {
  try {
