...
async function getSubmissionResult(token) {
  try {
    ...
    while (attempts < maxAttempts) {
      ...
      // Decode and normalize fields — treat literal 'null'/'undefined' as empty
      const _decode = (val) => {
        const d = decode(val).toString();
        const t = d.trim();
      ...
      return {
        ...
        compileOutput: _decode(result.compile_output),
        message: _decode(result.message),
