...
const runMigration = async () => {
  try {
    ...
    // Step 2: Firebase-specific migration - make password nullable
    ...
    if (passwordColumn.length > 0 && passwordColumn[0].IS_NULLABLE === 'NO') {
      await connection.query('ALTER TABLE users MODIFY COLUMN password TEXT NULL');
      console.log('✅ Password column is now nullable (Firebase integration)');
    } else {
      console.log('ℹ️ Password column is already nullable');
    }

    // Step 3: Read and run the remaining SQL statements from migration files
    const migrationFiles = [
      'add_quiz_duration.sql',
      'add_function_name.sql',
      'add_code_drafts.sql',
      'add_platform_links.sql',
